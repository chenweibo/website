$(function(){
	...
	var m_l=(1920-w_w)/2;
 	...
 	$('.nav_m .n_icon').click(function(){
		...
		$('.box-nav .nav').stop().slideUp();
	});
	
	// 手机内导航
	$('.box-nav .m-right').click(function() {
	  $('.box-nav .nav').stop().slideToggle();
	  $('.nav_m .nav').stop().slideUp();
	});
...
$(function(){
	...
	if($body_w > 960) {
		$("#menuA").hover(function(){
			$(".header-bg").stop().animate({"height":"382px"},200)
			$(".header").stop().animate({"height":"370px"},200)
		},function(){
			$(".header-bg").stop().animate({"height":"82px"},400)
			$(".header").stop().animate({"height":"70px"},400)
		});
		$("#menuA").hover(function(){
